                        ...
                        <form action="{{ isset($billingPackage) ? route('billing-packages.update', $billingPackage->id) : route('billing-packages.store') }}" method="POST">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select class="form-select select2 @error('report_id') is-invalid @enderror" 
                                            id="report_id" 
                                            name="report_id[]" 
                                            multiple>
                                        @foreach ($reports as $report)
                                            @php
                                                $selectedReports = old('report_id', is_string($billingPackage->report_id ?? '') ? json_decode($billingPackage->report_id ?? '[]', true) : ($billingPackage->report_id ?? []));
                                            @endphp
                                            <option value="{{ $report->id }}"
                                                {{ in_array($report->id, $selectedReports) ? 'selected' : '' }}>
                                                {{ $report->name }}
                                            </option>
                                        @endforeach
                                    </select>
